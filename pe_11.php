...
$range = 4;
$max = 0;
for( $row = 0; $row < sizeof($number_chart) - ($range-1); $row++):
	for( $col = 0; $col < sizeof($number_chart[$row]) - ($range-1); $col++):
		//horizontal and vertical
		for( $hvr = 0; $hvr < $range; $hvr ++):
			$a = 1; $b = 1;
			for( $hvc=0; $hvc < $range; $hvc++):
				$a = $a * $number_chart[$row+$hvr][$col+$hvc];
				$b = $b * $number_chart[$row+$hvc][$col+$hvr]; 
			endfor;
		...
		$a = 1; $b = 1;
		for( $d = 0; $d < $range; $d++):
			$a = $a * $number_chart[$row+$d][$col+$d];
			$b = $b * $number_chart[$row+$range-1-$d][$col+$d];
		endfor;
		if($a > $max): $max = $a; endif;
		if($b > $max): $max = $b; endif;
	endfor;
